    ...
    <div class="main-content-wrap">
        ...
        <div class="wg-box">
            <form class="form-new-product form-style-1" method="POST" action="{{route('admin.coupon.update')}}">
                ...
                <fieldset class="name">
                    <div class="body-title">Expired On <span class="tf-color-1">*</span></div>
                    <input class="flex-grow" type="date" placeholder="Expired On"
                        name="expired_on" tabindex="0" value="{{$coupon->expired_on}}" aria-required="true"
                        required="">
                </fieldset>
                @error('expired_on') <span class="alert alert-danger text-center">{{$message}}</span> @enderror
